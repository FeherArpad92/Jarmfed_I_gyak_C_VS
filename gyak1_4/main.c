...
int main(void)
{
    ...
    char celString[50]; // Fel kell készíteni egy célsztringet, ami elegendően nagy a forrás szöveg tárolásához.
    ...
    char *osszefuzottString = (char*)malloc(strlen(elsoString) + strlen(masodikString) + 10); // Először lefoglalunk egy elég nagy területet az összefűzött stringnek

    // Összefűzzük a két stringet
    // Először másoljuk az első stringet az összefűzött stringbe
    strcpy_s(osszefuzottString, strlen(elsoString) + strlen(masodikString) + 1, elsoString);
    // Majd hozzáfűzzük a második stringet
    strcat_s(osszefuzottString, strlen(elsoString) + strlen(masodikString) + 1, masodikString);

    // Kiírjuk az összefűzött stringet
    ...
    // Ellenőrizzük, hogy a szám pozitív-e
    if (szam < 0) {
